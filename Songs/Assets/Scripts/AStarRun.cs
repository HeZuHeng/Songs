...
public class AStarRun : MonoBehaviour
{
    ...
    private void Awake()
    {
        map = new Map((int)transform.position.x - 2000, (int)transform.position.z - 2000, (int)transform.position.x + 2000, (int)transform.position.z + 2000);
    }
    ...
    public Vector3[] AStarFindWay(Vector3 starPoint, Vector3 targetPoint)
    {
        ...
        open_List.Clear();
        close_List.Clear();
        map = new Map((int)transform.position.x - 50, (int)transform.position.z - 50, (int)transform.position.x + 50, (int)transform.position.z + 50);
        //初始化起点格子
    ...
    {
        //判断地图上某个坐标点是不是障碍点
        Vector3 p = new Vector3(x, transform.position.y + 1f, z);

        //检测该点周边是否有障碍物
        //障碍物层级为8
        Collider[] colliders = Physics.OverlapSphere(p, 0.3f, 1 << 0);
...
public class Map
{
    ...
    public Map(int star_X, int star_Z, int end_X, int end_Z)
    {
        this.star_X = star_X;
        this.star_Z = star_Z;
        this.end_X = end_X;
