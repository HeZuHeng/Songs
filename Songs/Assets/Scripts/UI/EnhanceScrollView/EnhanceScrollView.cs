...
public class EnhanceScrollView : MonoBehaviour
{
    ...
    public EnhanceItem curCenterItem;
    ...
    public void Refrshed()
    {
        ...
        {
            listEnhanceItems.Add(content.GetChild(i).GetComponent<EnhanceItem>());
            rect = content.GetChild(i) as RectTransform;
            w = rect.rect.width;
        }
        RefrshedStart();
        content.localPosition = new Vector3(-(w -cellWidth - 10)* listEnhanceItems.Count, content.localPosition.y, content.localPosition.z);
    }
